...
space_ssh_user = 'nobody'
space_host = 'localhost'
space_show_invoice_url = ''
space_pdf_invoice_url = ''
state_path = 'mig.data'
...
def jsonrpc(auth_token, apiname, **kw):
    ...
    context = migrated.location.get(location_id, 0)
    result = app.dispatch(auth_token, 0, params)
    if 'error' in result:
...
def import_image(filename):
    ...
    cmd = "scp %s@%s:/opt/apphomes/hubspace/hubspace/binaries/%s binaries/" % (space_ssh_user, space_host, filename)
    ret = os.system(cmd)
...
def download_invoices(old_id, new_id):
    ...
    show_invoice_url = space_show_invoice_url + '/%s'
    pdf_invoice_url = space_pdf_invoice_url + '/%s'
    paths = [(show_invoice_url, 'html'), (pdf_invoice_url, 'pdf')]
...
class Invoice(Object):
    ...
    def post(self):
        print("Invoice %s migration | post" % self.id)
...
def migrate_location():
    ...
    if 'membership' in objects_to_import:
        ...
        for id in member_ids_w_memberships:
            ...
            # Below was done to assign guest membership to members so that they appear in search
            # but now search is based on roles
    ...
    if 'role' in objects_to_import:
        for id in member_ids:
            ...
            q = "SELECT group_name from user_group, tg_group where user_id=%(member_id)s and group_id in (SELECT id from tg_group WHERE place_id = %(location_id)s and (group_name like '%%_member or %%_director' or group_name like '_%%host')) and tg_group.id = user_group.group_id"
    ...
    for exemption in exemptions:
        ...
        data = dict(member=migrated.member[exemption.user_id], tax_exemption_at=[new_location_id], applicable=True)
        jsonrpc(auth_token, 'billingpref.set_tax_exemption', **data)

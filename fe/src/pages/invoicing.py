...
class New(BasePage):
    ...
    def content(self):
        ...
        cell1 = tf.TD("Date")
        cell2 = tf.TD()
        cell2.input = tf.INPUT(type="date", value="11/11/2011")
        tr.cells = [cell1, cell2]
        info.tr = tr
        tr = tf.TR()
        cell1 = tf.TD("P. O. Number")
        cell2 = tf.TD()
        cell2.input = tf.INPUT(type="text", name="po_number", placeholder="Optional")
        tr.cells = [cell1, cell2]
        info.tr = tr
        ...
        content.notice.text = tf.TEXTAREA(name="message", placeholder="Test")

        usages = tf.TABLE(id='usages')
        ...
        usage_tmpl.tr.td = tf.TD('${start} - ${end}')
        usage_tmpl.tr.td = tf.TD('${total}')
        ...
        invoice_buttons.status = tf.DIV("Invoice is not saved yet.", id="action-status")
        invoice_buttons.buttons = [ tf.BUTTON("Save", Class="big-button"),
            tf.BUTTON("View (PDF)", Class="big-button"), tf.BUTTON("Send", Class="big-button")]

...
class ResourceCreate(BasePage):
    ...
    def content(self):
        ...
        form = sphc.more.Form(classes=['hform'], enctype='multipart/form-data', data_bind="submit: create_resource")
        form.add_field('Name', tf.INPUT(type='text', name='name').set_required())
        resource_types = tf.SELECT(name='type', data_bind="value: resource_type")
        ...
        form.add_field('Time based', tf.INPUT(type='checkbox', data_bind="disable: resource_type() == 'room', checked: time_based()"))
        form.add_field('Calendar', tf.INPUT(name='calendar', type='checkbox', data_bind='checked: calendar()'), "If checked, this resource will be available to members for booking on calendar page")
        form.add_field('Default Price', tf.INPUT(type="text", name='default_price').set_required(), "Will be added to Guest Tariff. Used in calculating cost of a usage by a person without a membership")
        form.add_field('Picture', tf.INPUT(name="picture", type="file", accept="image/*"), "Suggested Image Dimensions : 250x250.")
        form.add_field('Short Description', tf.TEXTAREA(name='short_description'))
        form.add_field('Long Description', tf.TEXTAREA(name='long_description'))

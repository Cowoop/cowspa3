...
class WeekAgenda(BookingPage):
    def content(self):
        ...
        bookings_tmpl.aday.date.year = tf.SPAN("${moment(iso2date(date)).format('dddd')}", Class="year")
        bookings_tmpl.aday.date.day = tf.SPAN("${iso2date(date).getDate()}", Class="day")
        bookings_tmpl.aday.date.month = tf.SPAN("${moment(iso2date(date)).format('MMMM')}", Class="month")

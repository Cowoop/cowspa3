...
def get_location_form():
    ...
    currency_select = tf.SELECT(id='currency', name='currency')
    for currency in data_lists.currencies:
        currency_select.option = tf.OPTION(currency['label']+" ("+currency['name']+")", value=currency['name'])
    form.add_field('Currency', currency_select)

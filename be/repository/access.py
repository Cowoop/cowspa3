...
def make_rstore(store):
    rstore = RStore()
    for attr in ('setup', 'add', 'add_many', 'remove', 'remove_by', 'remove_many', 'get', 'get_many', 'get_by', 'get_one_by', 'get_all', 'update', 'update_many', 'update_by', 'count', 'query_exec'):
...
def list_invoices(issuer ,limit):
    query = "SELECT invoice.number, member.name, invoice.cost, invoice.created as created, invoice.id, invoice.sent FROM member, invoice WHERE member.id = invoice.member AND issuer = %(issuer)s ORDER BY created DESC LIMIT %(limit)s"

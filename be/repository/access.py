...
def list_invoices(issuer ,limit):
    query = "SELECT invoice.number, member.name, invoice.cost, invoice.created as created, invoice.id FROM member, invoice WHERE member.id = invoice.member AND issuer = %(issuer)s ORDER BY created DESC LIMIT %(limit)s"
